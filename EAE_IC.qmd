...
wrap_plots(FeaturePlot(sc.norm, features = c('Cd4','Cd8a'),ncol = 1)&NoAxes())+clPlot 
...
Clusters 16, 11 and 17 still remain unclear. Checking by calculating unique cluster markers:
...
```{r}
#cluster 17 still looks like duplets, as it expresses both T-Cell and MG Markers and has a higher Feature Count the the rest of the dataset 
scCC.noDblRE %>% DimPlot_scCustom(label = TRUE, group.by = 'seurat_clusters') /
scCC.noDblRE %>% FeaturePlot(.,c('Cd3g','C4b','nFeature_RNA'))
```

```{r}
#cluster 19 is Ptgds+ Oligodendrocytes, cluster 20 is Cldn5+ Endothelial cells 
scCC.noDblRE %>% DimPlot_scCustom(label = TRUE, group.by = 'seurat_clusters') /
scCC.noDblRE %>% FeaturePlot(.,c('Ptgds','Cldn5'))
```

```{r,fig.asp = 0.5}
scCC.noDblRE.clean  = scCC.noDblRE[,scCC.noDblRE$seurat_clusters %!in% c(17,19,20)]
...
The following blocks generate the figures as found in the paper. If you want to skip the pre processing steps, you can access the final object from Geo and load it here
...
```{r}
#Microglia 
dfMgWt = FetchData(sc.mg,
                   vars = mgActMarker, 
                   slot = 'data',
                   cells = colnames(sc.mg)[sc.mg$sample =='EAE_IC_WT']) %>% 
        gather() %>% 
        mutate(genotype = 'WT')

dfMgHmbp = FetchData(sc.mg,
                     vars = mgActMarker, 
                     slot = 'data',
                     cells = colnames(sc.mg)[sc.mg$sample =='EAE_IC_HMBP']) %>%
           gather() %>% 
           mutate(genotype = 'hMBP')
...
```{r}
#T-Cells
dfTWt = FetchData(sc.t,
                  vars = tActMarker, 
                  slot = 'data',
                  cells = colnames(sc.t)[sc.t$sample =='EAE_IC_WT']) %>% gather() %>% mutate(genotype = 'WT')

dfTHmbp = FetchData(sc.t,
                    vars = tActMarker, 
                    slot = 'data',
                    cells = colnames(sc.t)[sc.t$sample =='EAE_IC_HMBP']) %>% 
          gather() %>% 
          mutate(genotype = 'hMBP')
...
tPlot = ggplot(dfT, 
               aes(x = factor(key,level = tActMarker), 
                   y = value,
...
```{r}
dfCkWt = FetchData(scCC.noDblRE.clean,
                   vars = cytokines, 
                   slot = 'data',
                   cells = colnames(scCC.noDblRE.clean)[scCC.noDblRE.clean$sample =='EAE_IC_WT']) %>% 
        gather() %>% 
        mutate(genotype = 'WT')

dfCkHmbp = FetchData(scCC.noDblRE.clean,
                     vars = cytokines, 
                     slot = 'data',
                     cells = colnames(scCC.noDblRE.clean)[scCC.noDblRE.clean$sample =='EAE_IC_HMBP']) %>% 
          gather() %>% 
          mutate(genotype = 'hMBP')
...
```{r}
dfMacWt = FetchData(sc.mac,
                    vars = macGenes, 
                    slot = 'data',
                    cells = colnames(sc.mac)[sc.mac$sample =='EAE_IC_WT']) %>% 
          gather() %>% 
          mutate(genotype = 'WT')

dfMacHmbp = FetchData(sc.mac,
                      vars = macGenes, 
                      slot = 'data',
                      cells = colnames(sc.mac)[sc.mac$sample =='EAE_IC_HMBP']) %>%
            gather() %>% 
            mutate(genotype = 'hMBP')
...
mgPlot %>% ggsave('mg.svg', plot = ., dpi = 600, width = 10, height = 4, units = 'cm')
mgPlot
```

```{r fig.width = 4.5,fig.height = 1}
(macPlot + tPlot + ckPlot) %>%ggsave('t_mac_ck.svg',plot = .,dpi = 600,width = 18,height = 4,units = 'cm')
...
ggsave('umap.svg',plot = umap,dpi = 600,width = 9.22,height = 8.2,units = 'cm')
umap
```
...
qcplot
ggsave('qc_plots.svg',qcplot,dpi = 600,width = 4,height = 12,units = 'cm')
```
...
ggsave('pca_pairsplot.svg',pcaPlot,dpi = 1200,width = 36,height = 36,units = 'cm')
```
...
celltypeDot
ggsave('celltypes_dotplot.svg',celltypeDot,dpi = 600,width = 11,height = 12.9,units = 'cm')
```
...
celltypeFeature
ggsave('celltypes_feature.svg',celltypeFeature,dpi = 600,width = 7,height = 12.9,units = 'cm')
